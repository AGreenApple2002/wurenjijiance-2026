...
本仓库被本地改造为无人机航拍小目标检测的结构实验工程。**以下内容不属于上游 ultralytics**，
任何新会话/智能体在改动代码前请先读：

- `CGBA_GUIDE.md` —— 改动索引（文件 + 行号）、模块设计与数据流、yaml 索引顺延对照、运行命令、
  实测数据（参数量 / GFLOPs / PyTorch 与 TensorRT 延迟 / 数值一致性）、踩坑清单；
- `EXPERIMENT_NOTES.md` —— 实验记录与 TensorRT 部署困难清单；
- `.github/copilot-instructions.md` —— 本地会话速查与硬性约束。

改动速查：

- 新增模块：`ultralytics/nn/modules/cgba.py`（`LBDown` 可学习双边下采样、`CGBlockAttn` 粗粒度块注意力）；
- 新增配置：`ultralytics/cfg/models/26/yolo26-cgba.yaml`（对照物为官方 `yolo26.yaml`）；
- 注册点：`ultralytics/nn/modules/__init__.py`、`ultralytics/nn/tasks.py` 的 `parse_model` `base_modules`；
- 实验脚本：`check_cgba.py`、`bench_trt.py`、`export_cgba_onnx.py`、`tensorRT-test.py`、`trt_infer_pure.py`、`trt_webcam.py`。

硬性约束：**不得报出任何精度 / mAP / 召回数字**（未做训练与数据集验证）；改动层结构后必须顺延
`yolo26-cgba.yaml` 中 head 的 `Concat` from 索引，并重跑 `check_cgba.py` 与 `bench_trt.py`。
